...
class Ion_auth_model extends CI_Model {
	...
	public function __construct()
	{
		...
		$this->identity_alt_column = $altIdCol !== FALSE ? $altIdCol : '';
	 ...
	 * @author Mathew
	 **/
	public function deactivate($id = NULL, $memo = NULL)
	{
		...
		$data = array(
		    ...
		    'active'          => 0, //deactivate diganti diputihkan
			//'alasan_non_aktif'=> $memo
		);
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $users_data = array(),
		...
		// Users table.
		$data = array(
		    $this->identity_alt_column   => $identity,
		    'password'   => $password,
		...
		$user_data = array_merge($this->_filter_data($this->tables['users'], $users_data), $data);
		$this->trigger_events('extra_set');
		$this->db->insert($this->tables['users'], $user_data);

		$id = $this->db->insert_id();
		
		/* insert table users_detail (ubah) */
		$users_detail = array_merge($this->_filter_data('users_detail', $users_detail_data));
		$this->db->set('id_user', $id);
		$this->db->insert('users_detail', $users_detail_data);
		
		//$users_detail = array( 'id_user' => $id ); $this->db->insert('users_detail', $users_detail); // 

		// add in groups array if it doesn't exits and stop adding into default group if default group ids are set
		if( isset($default_group->id) && empty($groups) )
		{
			//$groups[] = $default_group->id; 
			//ini dimatikan biar group default ga ada, tapi langsung di pilih groupnya
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->trigger_events('extra_where');
		$identity_column = ($identity ? $this->identity_column : $this->identity_alt_column);
		
		//$check_column = valid_email($identity) ? $this->identity_column : 'username';  
		$query = $this->db->select($this->identity_column . ', ' . $this->identity_alt_column . ', 
																users.full_name, 
																users.first_name, 
																users.last_name,
																users.email, 
																...
																//users.alasan_non_aktif,
																//users_detail.nia'
						  ->join('users_detail', 'users.id = users_detail.id_user', 'left')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				// jika akun masih baru atau belum aktif
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
				...
				// jika akun di putihkan
				if ($user->active == 2)
				{
					$ana = $user->alasan_non_aktif;
					$this->set_error($ana);
	 ...
	 * @author Ben Edmunds
	 **/
	public function user($id = NULL, $username_url = NULL)
	{
		...
		if($username_url){$this->where($this->tables['users'].'.id', $username_url);}
	 ...
	 **/
	public function update($id, array $data, array $detail)
	{
		...
		$this->db->update('users_detail', $detail, array('id_user' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['users_detail'], array('id' => $id));  // table users_detail
	...
	public function update_last_login($id)
	{
		...
		// Date by Ahmad Romadhon
		$now = date('Y-m-d H:i:s');
		$unix = human_to_unix($now);
		
		//$this->db->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
		$this->db->update($this->tables['users'], array('last_login' => $unix), array('id' => $id));
