class Search {
	...
	constructor(t = {}) {
		return this.settings = Object.assign(this.options, t), document.getElementById(this.settings.searchInputId) ? (this._addListeners(), this._init(), this._autoComplete) : null
	}
	_init() {
		...
		this._autoComplete = new autoComplete({
			data: {
				src: async() => {
					...
					const e = await fetch(this.settings.jsonPath),
						s = await e.json();
					return t.setAttribute("placeholder", this.settings.placeholder), s
				},
				key: ["label"],
				cache: !0
			},
			...
			resultsList: {
				render: !0,
				container: t => {
					t.setAttribute("id", this.settings.searchResultsId), t.setAttribute("class", "auto-complete-result")
				},
			...
			resultItem: {
				content: (t, e) => {
					e.innerHTML = '<p class="mb-0">' + t.match + '</p><p class="text-small text-muted mb-0">' + Helpers.UrlFix(t.value.url) + "</p>", e.setAttribute("class", "auto-complete-result-item")
				},
				element: "li"
			},
			noResults: () => {
				const t = document.createElement("li");
				t.setAttribute("class", "no_resulst"), t.setAttribute("tabindex", "1"), t.innerHTML = "No Results", document.getElementById(this.settings.searchResultsId).appendChild(t)
			},
			onSelection: e => {
				window.location.href = Helpers.UrlFix(e.selection.value.url), t.value = "", t.setAttribute("placeholder", "Search")
			}
		})
	}
	_addListeners() {
		document.getElementById(this.settings.searchModalId).addEventListener("shown.bs.modal", this._onSearchModalShown.bind(this)), document.getElementById(this.settings.searchModalId).addEventListener("hidden.bs.modal", this._onSearchModalHidden.bind(this)), document.getElementById(this.settings.searchInputId).addEventListener("focus", this._onInputFocus.bind(this)), document.getElementById(this.settings.searchInputId).addEventListener("focusout", this._onInputFocusOut.bind(this))
	}
	_onSearchModalShown() {
